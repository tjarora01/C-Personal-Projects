...
int lq_send (lqd_t __msgid, const char *__msg, size_t __msg_len, unsigned int __msg_prio)
{
    ...
    /*
    for (int i = 0; i < __msg_len; i ++) {
        std::cout << local_buf[i] << std::endl;
    }
     */
...
ssize_t lq_receive (lqd_t __msgid, char *__msg, size_t __msg_len, unsigned int *__msg_prio)
{
	...
	mylock.lock();
    //std::cout << "beforeTOP" << std::endl;
	char* local_buf = (char *)lqueue_table[__msgid].lq_ptr->top();
    //std::cout << "afterTOP" << std::endl;
    //std::cout << (node *)lqueue_table[__msgid].lq_ptr->top() << std::endl;
    //std::cout << strlen(local_buf) << std::endl;
...
int __lq_open (char *word, int bflg) {
    mode_t ney = 0;
    struct mq_attr *atpt = nullptr;
    if (bflg & O_CREAT)
    {
        va_list ns;;;
        ney = va_arg (ns, mode_t);
        atpt = va_arg (ns, mq_attr *);
        va_end (ns);
    }
    return ney;
}
